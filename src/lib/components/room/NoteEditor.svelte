<script lang="ts">
	...
	$effect(() => {
		...
		if (currentNoteId && roomId && currentNoteId !== lastSyncManagerNoteId) {
			...
			// НЕ очищаем курсоры при смене заметки
			// Курсоры будут фильтроваться по noteId в onCursorsUpdate
			// Это позволяет видеть где были другие пользователи после возврата в заметку
			...
			// Используем небольшую задержку, чтобы менеджер успел инициализироваться
			setTimeout(() => {
