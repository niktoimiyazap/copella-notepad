<script lang="ts">
	...
	$effect(() => {
		...
		if (currentNoteId && roomId && currentNoteId !== lastSyncManagerNoteId) {
			...
			// ОЧИЩАЕМ курсоры при смене заметки
			// Они появятся заново когда другие пользователи в этой заметке отправят свои позиции
			...
			// Отправляем начальную позицию курсора при заходе в заметку
			// Минимальная задержка для инициализации менеджера
			setTimeout(() => {
				if (diffSyncManager && editorElement && isFocused) {
	...
	onMount(() => {
		...
		// НЕ удаляем курсор при скрытии страницы
		// Курсоры сохраняются 30 секунд и пользователи могут видеть где были другие
		// даже если они временно покинули страницу
	});

	onDestroy(() => {
		// НЕ удаляем курсор при уничтожении компонента
		// Это позволяет сохранить курсор при переключении между заметками
