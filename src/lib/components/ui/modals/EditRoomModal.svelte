<script lang="ts">
	...
	let { isOpen, onClose, onSubmit, roomData }: EditModalProps = $props();
	
	// Инициализируем состояние формы
	let formState = $state(initializeRoomFormState(roomData));
	...
	// Обновляем форму при изменении roomData
	$effect(() => {
		if (roomData) {
			Object.assign(formState, {
				...
				isLoading: false
				// НЕ сбрасываем uploadedImage и uploadedImageUrl
			});
		}
	});
</script>

<Modal {isOpen} onClose={formActions.handleClose} title="Редактировать комнату">
	<form onsubmit={formActions.handleSubmit} class="edit-room-form">
		<!-- Название комнаты -->
		<div class="form-group">
			<label for="room-title-edit" class="form-label">Название комнаты</label>
			<input
				id="room-title-edit"
				type="text"
		...
		<div class="form-group">
			...
			<div class="cover-upload-section">
				{#if formState.uploadedImageUrl || formState.coverImage}
					<div class="cover-preview-container">
						<img src={formState.uploadedImageUrl || formState.coverImage} alt="Cover preview" class="cover-preview-image" />
						<button
							type="button"
				...
				{:else}
					<div class="cover-upload-area">
						...
						{:else}
							...
							/>
							<label for="cover-upload-edit" class="cover-upload-label">
	...
	<div class="form-group">
		<label for="participant-limit-edit" class="form-label">Лимит участников</label>
		<div class="range-container">
			<input
				id="participant-limit-edit"
				type="range"
				min="1"
				max="50"
